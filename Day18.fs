...
let firstStack n =
    [ for x in 0..n do for y in 0..n -> (x, y) ] |> Set
    |> Seq.fold (flip Set.remove)

let splitAt n = Seq.take n <|> Seq.skip n

let astar  m =
    let around p = Seq.map ((++) p) [ -1, 0; 1, 0; 0, 1; 0, -1 ]
...
let blocks d m =
    ...
    |>| Seq.zip
    >> Seq.find (snd >> path d >> Option.isNone) >> fst

let part1 =
    let length = Option.get >> Seq.length >> minus1
    Seq.take 1024 >> firstStack 70 >> path 70 >> length

let part2 = splitAt 1024 >> mapFst (firstStack 70) >> uncurry (blocks 70)
